...
class Solver(object):
    ...
    def resume_checkpoint_teacher(self,model,resume_checkpoint):
        if resume_checkpoint == '' or not os.path.isfile(resume_checkpoint):
            print(("=> no checkpoint found at '{}'".format(resume_checkpoint)))
            return False
        print(("=> loading checkpoint '{:s}'".format(resume_checkpoint)))
        ...
        resume_scope = cfg.TRAIN.RESUME_SCOPE+',extra'
        # extract the weights based on the resume scope
        if resume_scope != '':
            pretrained_dict = {}
            for k, v in list(checkpoint.items()):
    ...
    def train_epoch(self, model, data_loader, optimizer, criterion, writer, epoch, use_gpu):
        ...
        for iteration in iter(range((epoch_size))):
            ...
            loss = loss_l + loss_c+l2_loss_weight*loss_l2_1 +(1-l2_loss_weight)*loss_l2_2
            loss.backward()
            optimizer.step()
            ...
            l2_loss+=l2_loss_weight*loss_l2_1.data[0] +(1-l2_loss_weight)*loss_l2_2.data[0]
            # log per iter
            log = '\r==>Train: || {iters:d}/{epoch_size:d} in {time:.3f}s [{prograss}] || loc_loss: {loc_loss:.4f} cls_loss: {cls_loss:.4f} l2_loss: {l2_loss:.4f}\r'.format(
                    ...
                    time=time, loc_loss=loss_l.data[0], cls_loss=loss_c.data[0],l2_loss=l2_loss_weight*loss_l2_1.data[0]+(1-l2_loss_weight)*loss_l2_2.data[0])
        ...
        log = '\r==>Train: || Total_time: {time:.3f}s || loc_loss: {loc_loss:.4f} conf_loss: {conf_loss:.4f} l2_loss: {l2_loss:.4f} || lr: {lr:.6f}\n'.format(lr=lr,
                time=_t.total_time, loc_loss=loc_loss/epoch_size, conf_loss=conf_loss/epoch_size,l2_loss=l2_loss/epoch_size)
        sys.stdout.write(log)
        sys.stdout.flush()
