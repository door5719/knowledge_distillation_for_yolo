...
class Detect(Function):
    ...
    def forward(self, predictions):
        ...
        decode_time=0
        _t['misc'].tic()
        #time_all={'decode':[], 'misc':[], 'box_mask':[], 'score_mask':[],'nms':[], 'cpu':[],'sort':[]}
        # Decode predictions into bboxes.
        for i in range(num):
            _t['decode'].tic()
            decoded_boxes = decode(loc_data[i], prior_data, self.variance)
            ...
            for cl in range(1, self.num_classes):
                ...
                ids, count = nms(boxes, scores, self.nms_thresh, self.top_k)
                print ("bbox num",len(boxes))
                gpunms_time += _t['nms'].toc()
                output[i, cl, :count] = \
                    torch.cat((scores[ids[:count]].unsqueeze(1),
                               boxes[ids[:count]]), 1)
        nms_time= _t['misc'].toc()
	#time_all['nms'].append(nms_time)
	#time_all['cpu'].append(cpu_tims)
        #time_all['box_mask'].append()
       # print(nms_time, cpu_tims, scores_time,box_time,gpunms_time)
        # flt = self.output.view(-1, 5)
        # _, idx = flt[:, 0].sort(0)
        # _, rank = idx.sort(0)
        # flt[(rank >= self.top_k).unsqueeze(1).expand_as(flt)].fill_(0)
        return output, nms_time, cpu_tims, scores_time,box_time,gpunms_time
