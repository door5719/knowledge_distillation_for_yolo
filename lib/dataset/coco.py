...
class COCODetection(data.Dataset):
    ...
    def __init__(self, root, image_sets, preproc=None, target_transform=None,
        ...
        self.OfficeDataset = [
            'head_difficult_train' , 'head_office_train',          # 5k val2014 subset
            'head_squatting_train' , 'head_steve_train',  # val2014 \setminus minival2014
        ...
        for image_set in image_sets:
            if image_set in self.OfficeDataset:
               ...
               self.root=os.path.join(root,'tracking_data/training_only','coco_crowdhuman')
               coco_name = 'annotations'
               self.data_name='coco_crowdhuman_'

            elif image_set in self.failed_frames:
               self.root=os.path.join(root,'tracking_data/training_only','failed_frames')
               coco_name = 'annotations'
               self.data_name='failed_frames_'

            elif image_set in self.training_data_resources1:
               self.root=os.path.join(root,'tracking_data/training_only','training_data_resources1')
               coco_name = 'annotations'
               self.data_name='training_data_resources1_'

            elif image_set in self.training_data_resources2:
               self.root=os.path.join(root,'tracking_data/training_only','training_data_resources2')
               coco_name = 'annotations'
               self.data_name='training_data_resources2_'

            elif image_set in self.training_data_resources3:
               self.root=os.path.join(root,'tracking_data/training_only','training_data_resources3')
               coco_name = 'annotations'
    ...
    def image_path_from_index(self, name, index):
        ...
        if not os.path.exists(image_path):
           image_path = os.path.join(self.root, 'frames',file_name)

        assert os.path.exists(image_path), \
                'Path does not exist: {}'.format(image_path)
        #print ('aifi_image_path',image_path)
        return image_path
    ...
    def _load_coco_annotations(self, coco_name, indexes, _COCO):
        cache_file=os.path.join(self.cache_path,self.data_name+coco_name+'_gt_roidb.pkl')
        if os.path.exists(cache_file):
            with open(cache_file, 'rb') as fid:
                roidb = pickle.load(fid)
            print('{} gt roidb loaded from {}'.format(self.data_name+coco_name,cache_file))
            return roidb

        print('parsing gt roidb for {}'.format(self.data_name+coco_name))
    ...
    def _load_coco_img_path(self, coco_name, indexes):
        cache_file=os.path.join(self.cache_path,self.data_name+coco_name+'_img_path.pkl')
        if os.path.exists(cache_file):
            with open(cache_file, 'rb') as fid:
                img_path = pickle.load(fid)
            print('{} img path loaded from {}'.format(self.data_name+coco_name,cache_file))
            return img_path

        print('parsing img path for {}'.format(self.data_name+coco_name))
        img_path = [self.image_path_from_index(self.data_name+coco_name, index)
                    for index in indexes]
        with open(cache_file, 'wb') as fid:
